...
from MakimaRobot import OWNER_ID, SUPPORT_CHAT, telethn
...
async def lego(event):
    ...
    if event.sender_id != OWNER_ID and not quew:
        await event.reply(
            "ɢɪᴠᴇ sᴏᴍᴇ ᴛᴇxᴛ ᴛᴏ ᴄʀᴇᴀᴛᴇ ʟᴏɢᴏ ʙᴀʙʏ​ !\nExample : `/logo 𝓚𝒶кคѕⒽᎥ ђ𝔞𝓉ᗩЌ𝒆 ⸙『𝕭𝖎𝖓𝖌𝖊』 ᭄™`"
        )
        return
    ...
    try:
        ...
        fnt = glob.glob("./MakimaRobot/resources/fonts/*")
        ...
        await telethn.send_file(
            event.chat_id,
            file=fname,
            caption=f"ʟᴏɢᴏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ [Makima](https://t.me/Makima_UltraProxbot)",
        )
        await pesan.delete()
